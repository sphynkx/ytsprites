...
from .worker_srv import start_workers


def serve():
    # New protocol doesn't transfer big payloads to/from client (only metadata),
    # but keep a reasonable limit anyway.
    max_msg_size = max(16, int(getattr(cfg, "MAX_VIDEO_SIZE_MB", 64))) * 1024 * 1024
    options = [
        ("grpc.max_receive_message_length", max_msg_size),
        ("grpc.max_send_message_length", max_msg_size),
    ]

    # Start background workers BEFORE server starts accepting requests
    start_workers()
